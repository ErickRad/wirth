...
$(UEFI_EFI_EXE): boot/grub.cfg | $(BUILD_DIR) check-iso-tools
	grub-mkstandalone -O x86_64-efi -d $(GRUB_EFI_DIR) \
		--modules="part_gpt part_msdos fat iso9660 multiboot2 normal configfile search" \
		-o $@ "boot/grub/grub.cfg=boot/grub.cfg"
...
$(ISO_IMAGE): toolchain $(KERNEL_ELF) $(KERNEL64_ELF) $(UEFI_BOOT_IMAGE) boot/grub.cfg check-iso-tools
